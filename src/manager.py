...
class Manager:
    ...
    # -------------------------------
    # Funkcjonalność A (walidacja transferów)
    # -------------------------------
    def validate_transfer_amount(self, amount: float, min_value: float = 0.0, max_value: float = 10000.0) -> bool:
        if amount < min_value or amount > max_value:
            raise ValueError(f"Kwota transferu {amount} jest poza zakresem {min_value}-{max_value}")
        return True

    # -------------------------------
    # Funkcjonalność B (czarna lista najemców)
    # -------------------------------
    def get_blacklist(self):
        # Domyślnie zwracamy pustą listę, w testach można podmienić przez monkeypatch
        return []

    def is_tenant_blacklisted(self, tenant):
        for entry in self.get_blacklist():
            if entry["first_name"] == tenant["first_name"] and entry["last_name"] == tenant["last_name"]:
                return True
        return False

    # -------------------------------
    # Funkcjonalność C i pozostałe metody
    # -------------------------------
